...
class JobMatcher:
    ...
    def find_top_matches(self, user_id: int, limit=5, user_query_text=None):
        ...
        print(
            f"🔍 [Matcher] Found {len(candidates)} candidates from DB to scan."
        )  # DEBUG
        ...
        for job in candidates:
            ...
            if semantic_score > 0:
                ...
                final_score = (
                    keyword_score  # Nếu không có vector, dùng 100% điểm keyword
                )

            if final_score > 0:
                print(
                    f"   -> Job: {job.title} | Key: {keyword_score:.1f} |"
                    f" Sem: {semantic_score:.1f} | Final: {final_score:.1f}"
                )
                ...
                scored_jobs.append(
                    {
                        ...
                        "reason": (
                            "Phù hợp ngữ nghĩa & chuyên môn"
                            if semantic_score > 0
                            else "Phù hợp từ khóa kỹ năng"
                        ),
                    }
                )
